...
const swalCustom = Swal.mixin({
    customClass: {
      ...
      denyButtonText:'btn btn-danger'
    },
    buttonsStyling: true
});

let serverDomain = 'peaceful-hollows-08391.herokuapp.com';
const isHttps = ( serverDomain == "localhost" ) ? false:true ; // must be the same to server.js isHttps
let signalingServerPort = 3000; // must be the same to server.js PORT
const signalingServer = getSignalingServer();
const roomId = getRoomId();
const peerInfo = getPeerInfo();
const peerLoockupUrl = 'https://extreme-ip-lookup.com/json/?key=demo';
const avatarApiUrl = 'https://eu.ui-avatars.com/api';
const welcomeImg = '../images/Connect_Us.png';
const shareUrlImg = '../images/Connect_Us.png';
const leaveRoomImg = '../images/leave-room.png';
const confirmImg = '../images/Connect_Us.png';
const fileSharingImg = '../images/Connect_Us.png';
...
let recStartTime;
let recElapsedTime;
let connectusTheme = 'ghost'; // neon - dark - forest - ghost ...
let connectusBtnsBar = 'horizontal'; // vertical - horizontal
let swalBackground = 'rgba(255, 255, 255, 1)'; // black - #16171b - transparent ...
let peerGeo;
let peerConnection;
let myPeerName;
let myPeerPic;
...
function getHtmlElementsById() {
    ...
    themeSelect = getId('connectusTheme');
    btnsBarSelect = getId('connectusBtnsBar');
...
function setButtonsTitle() {
    ...
    tippy(lockUnlockRoomBtn, {
        content: 'Meeting Lock Button',
        placement: 'right-start',
    });
...
function getSignalingServer() {
    ...
    getServerPort().then((data) => {
        signalingServerPort=data;
        console.log(`Client is listening at the port: ${signalingServerPort}`);
        if (isHttps) {
            return 'https://' + serverDomain + ':' + signalingServerPort; 
 ...
 */
 function whoAreYou() {
    ...
    getPeerName().then((data) => {
        myPeerName = data.name;
        myPeerPic = data.picture;
        myVideoParagraph.innerHTML = myPeerName + ' (me)';
        // setPeerImg('myVideoAvatarImage', data.picture);
        setPeerImg('myVideoAvatarImage', myPeerPic);
        setPeerChatAvatarImgName('right', myPeerName);
        joinToChannel();
        // userLog('toast', myPeerName + ' Joined the Room');

        console.log("entering meet");
        // welcomeUser();
    })

    setMyAudioOff(myPeerName);
...
//         position: 'center',
//         imageAlt: 'Connect Us-name',
//         imageUrl: welcomeImg,
//         title: 'Choose your media settings',
//         // input: 'text',
//         html: `<br>
//         <div style="overflow: hidden;">
//             <button id="initAudioBtn" class="fas fa-microphone-alt" onclick="handleAudio(event, true)"></button>
...
function welcomeUser() {
    ...
    swalCustom.fire({
        ...
        if (result.isConfirmed) {
            ...
            let message = {
                email: '',
                subject: 'Please join our Connect us Video Chat Meeting',
        ...
        event.channel.onmessage = (msg) => {
            switch (event.channel.label) {
                case 'connect_us_chat_channel':
                    ...
                    break;
                case 'connect_us_file_sharing_channel':
                    try {
...
function handleDisconnect() {
    ...
    // userLog('toast', myPeerName + ' has left the Room');
    chatDataChannels = {};
    fileDataChannels = {};
    peerConnections = {};
    peerMediaElements = {};
...
/**
 * Set connectus theme neon | dark | forest | sky | ghost | ...
 * @param {*} theme
 */
function setTheme(theme) {
    if (!theme) return;

    connectusTheme = theme;
    switch (connectusTheme) {
        case 'neon':
            // neon theme
    ...
    setButtonsBarPosition(connectusBtnsBar);
}
...
function setButtonsBarPosition(position) {
    ...
    connectusBtnsBar = position;
    switch (connectusBtnsBar) {
        case 'vertical':
            let btnsLeft = connectusTheme === 'ghost' ? '5px' : '20px';
...
function loadLocalMedia(stream) {
    ...
    myVideoParagraph.style="font-size:15px;";
    ...
    myAudioStatusIcon.className = 'fas fa-microphone-alt';
...
function loadRemoteMediaStream(stream, peers, peer_id) {
    ...
    let peer_pic = peers[peer_id]['peer_pic'];
    ...
    remoteAudioStatusIcon.className = 'fas fa-microphone-alt';
    ...
    setPeerImg(peer_id + '_avatar', peer_pic);
...
function setPeerImg(videoAvatarImageId, photoUrl) {
    let videoAvatarImageElement = getId(videoAvatarImageId);
    // default img size 64 max 512
    let sz = isMobileDevice ? 128 : 256;
    photoUrl = photoUrl.substring(0, photoUrl.lastIndexOf("=")) + "=s" + sz + "-c";
    videoAvatarImageElement.setAttribute(
        'src',
        photoUrl,
    );
    videoAvatarImageElement.style.borderRadius = "50%";
    videoAvatarImageElement.style.border = "1px solid white";
    videoAvatarImageElement.classList.remove("pulsate");
}
...
function setPeerChatImg(avatar, photoUrl) {
    let sz = isMobileDevice ? 128 : 256;
    photoUrl = photoUrl.substring(0, photoUrl.lastIndexOf("=")) + "=s" + sz + "-c";
...
function setChatRoomBtn() {
    ...
    //     if (connectusTheme == 'ghost') return;
    ...
    // console.log("hero, host priviledge -> youtube only host can share");
    ...
    if (!isSupportedNavigatorShare || (isSupportedNavigatorShare && errorNavigatorShare)) {
        ...
        swalCustom.fire({
            ...
            <div id="qrRoomContainer">
                <img src="../images/Connect_Us.png" style="width:60%;">
            </div>
            <br/><br/>
            <p style="color:white;"> Share this meeting invite others to join.</p>
            <p style="color:rgb(8, 189, 89);">` +
...
function copyRoomURL() {
    ...
    userLog('toast', 'Meeting URL is copied');
}
...
function handleAudio(e, init) {
    ...
    e.target.className = 'fas fa-microphone-alt' + (myAudioStatus ? '' : '-slash');
    if (init) {
        audioBtn.className = 'fas fa-microphone-alt' + (myAudioStatus ? '' : '-slash');
        if (!isMobileDevice) {
            tippy(initAudioBtn, {
...
function refreshMyLocalStream(stream, localAudioTrackChange = false) {
    ...
    if (localAudioTrackChange && myAudioStatus === false) {
        audioBtn.className = 'fas fa-microphone-alt';
...
function createChatDataChannel(peer_id) {
    chatDataChannels[peer_id] = peerConnections[peer_id].createDataChannel('connect_us_chat_channel');
...
function handlePeerName(config) {
    ...
    // userLog('toast', peer_name + ' joined the Room');
...
function setMyAudioStatus(status) {
    myAudioStatusIcon.className = 'fas fa-microphone-alt' + (status ? '' : '-slash');
...
function setPeerAudioStatus(peer_id, status) {
    ...
    console.log(peer_id,status);
    console.log(peerAudioStatus);
    console.log(peerAudioStatus.className);
    console.log("4444444444444444444");
    peerAudioStatus.className = 'fas fa-microphone-alt' + (status ? '' : '-slash');
...
function handlePeerAudioBtn(peer_id) {
    ...
    peerAudioBtn.onclick = () => {
        if (peerAudioBtn.className === 'fas fa-microphone-alt') disablePeer(peer_id, 'audio');
    };
}
...
function setPeerVideoStatus(peer_id, status) {
    ...
    console.log(peer_id,status);
    console.log(peerVideoStatus);
    console.log(peerVideoStatus.className);
...
function setMyAudioOff(peer_name) {
    ...
    myAudioStatus = localMediaStream.getAudioTracks()[0].enabled;
    audioBtn.className = 'fas fa-microphone-alt-slash';
    setMyAudioStatus(myAudioStatus);
    if (peer_name) 
...
function handleRoomStatus(config) {
    ...
    const rStatus = roomLocked ? '🔒 LOCKED the room, no one can access!' : '🔓 UNLOCKED the room';
    userLog('toast', peer_name + ' ' + rStatus)
}
...
function createFileSharingDataChannel(peer_id) {
    fileDataChannels[peer_id] = peerConnections[peer_id].createDataChannel('connect_us_file_sharing_channel');
...
function selectFileToShare() {
    ...
    swalCustom.fire({
        allowOutsideClick: false,
        background: swalBackground,
        imageAlt: 'connect_us-file-sharing',
    ...
    if (isImageURL(incomingFileInfo.fileName)) {
        ...
        reader.onload = (e) => {
            swalCustom.fire({
                ...
                imageAlt: 'connect_us-file-img-download',
                showDenyButton: true,
        ...
        swalCustom.fire({
            allowOutsideClick: false,
            background: swalBackground,
            imageAlt: 'connect_us-file-download',
            imageUrl: fileSharingImg,
            position: 'center',
            title: 'Received file',
            text: incomingFileInfo.fileName + ' size ' + bytesToSize(incomingFileInfo.fileSize),
...
function handlePeerKickOutBtn(peer_id) {
    ...
    // console.log("HERO, add host priviledge here");
...
function getPeerName() {
    return $.getJSON("/api/user").then((data) => {
